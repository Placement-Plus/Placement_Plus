...
const AlumniRegistrationScreen = () => {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      // Add current company
      formData.append("currentCompany.name", careerInfo.currentCompany.name);
      formData.append("currentCompany.position", careerInfo.currentCompany.position);

      // Add previous companies
      careerInfo.previousCompanies.forEach((company, index) => {
        formData.append(`previousCompany[${index}].name`, company.name);
        formData.append(`previousCompany[${index}].Position`, company.position);
        formData.append(`previousCompany[${index}].Duration`, company.duration);
        formData.append(`previousCompany[${index}].Experience`, company.experience);
      });
      ...
      const response = await FileSystem.uploadAsync(
        `http://${process.env.EXPO_PUBLIC_IP_ADDRESS}:5000/api/v1/alumni/register`,
        profilePic.uri,
        {
          ...
          parameters: {
            ...personalInfo,
            currentCompanyName: careerInfo.currentCompany.name,
            currentCompanyPosition: careerInfo.currentCompany.position,
            previousCompanies: JSON.stringify(careerInfo.previousCompanies)
          }
        }
      );
      ...
      if (result.statusCode === 200) {
        ...
        await alumniLogin(result?.data?.createdAlumni);
